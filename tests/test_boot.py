...
BOOT = [name for name in (
    ...
    '_migrate_contact_roles', '_migrate_retire_client', '_migrate_transaction_kinds', '_migrate_fold_unit_names',
...
assert len(BOOT) >= 16, f'only {len(BOOT)} boot steps found in serve.py'
with A.app.app_context():
    for name in BOOT:
        step = getattr(A, name)
        try:
